...
void showBoard(char tab[3][3])
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		for (int k = 0; k < 3; k++)
		{
			if(tab[i][k]=='O')cout << dye::green(tab[i][k]) << " ";
			else if (tab[i][k] == 'X')cout << dye::aqua(tab[i][k]) << " ";
			else cout << tab[i][k] << " ";
		}
		cout << "|\n";
	}
...
void yourMove(int XorO)
{
	if (XorO % 2 == 0)
	{
		cout << "Pick a field from 1 - 9 for an "<< dye::green("O") << ":" << " ";
	}
	else if (XorO % 2 != 0)
	{
		cout << "Pick a field from 1 - 9 for an " << dye::aqua("X") << ":" << " ";
	}
}
